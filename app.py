...
def is_sport_open(sport_name: str) -> bool:
    return get_setting(f"sport_closed::{sport_name}", "0") != "1"
...
def get_sports_config() -> dict[str, dict[str, object]]:
    ...
    for sport_name, config in BASE_SPORTS.items():
        ...
        sports[sport_name] = {
            ...
            "is_open": is_sport_open(sport_name),
        }
...
def get_registration_open_status_for_sport(sport_name: str | None) -> tuple[bool, str]:
    general_open, general_message = get_global_registration_open_status()
    if not general_open:
        return False, general_message

    if sport_name and not is_sport_open(sport_name):
        return False, f"Le iscrizioni per {sport_name} sono chiuse."
...
@login_required
def toggle_sport(sport_name: str):
    if sport_name not in BASE_SPORTS:
        flash("Sport non valido.", "error")
        return redirect(url_for("admin_dashboard"))

    current = get_setting(f"sport_closed::{sport_name}", "0")
    if current == "1":
        set_setting(f"sport_closed::{sport_name}", "0")
        flash(f"{sport_name} riaperto.", "success")
    else:
        set_setting(f"sport_closed::{sport_name}", "1")
        flash(f"{sport_name} chiuso.", "success")
